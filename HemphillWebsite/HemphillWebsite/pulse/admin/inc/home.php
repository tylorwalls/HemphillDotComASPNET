...
if ($_GET['f'] === 'stats') { include('view-stats.php'); }
			...
			if (!empty($val[0])){ 	
				...
				echo "<div class='file-row'>";
				 ...
				 if (in_array($ext2, $browser_files) && file_exists("../content/$folder/$base.$ext") ){
					$destination = "$path/content/$folder/$base.$ext";		
					echo "<a class='file-image' target='_blank' href='$destination'> $base.$ext </a>";
	             	echo "<a class='delete' href='index.php?p=delete&d=media/$fname[1]/$base&e=$ext'><img src='img/icon-delete.svg'></a>";    

				 }
				else{
					echo "<a class='file-image' href='index.php?p=open&f=media/$fname[1]/$base&e=$ext'> <img src='$path/content/$folder/$base.$ext'> </a>";
					echo "<a class='delete' href='index.php?p=delete&d=media/$fname[1]/$base&e=$ext'><img src='img/icon-delete.svg'></a>";
				}
				echo "</div>";
				?> </li> <?php 
    ...
    if ($fname[0] == 'media' && !empty($fname[1])){
			...
			<?php
	    	 ...
	    	 if ($fname[0] == 'media') { echo "<a href='#' class='tooltip' alt='$lang_gallery_tool_tip'>?</a>"; }
	    	}
	...
	if ($fname[0] == 'blog'){ 	    
	    echo "<a class='btn blog_preview-button' target='_blank' href='$blog_url'>$lang_home_preview</a>"; 
	     if ($fname[0] == 'blog') { echo "<a href='#' class='tooltip' alt='$lang_blog_tool_tip'>?</a>"; }
