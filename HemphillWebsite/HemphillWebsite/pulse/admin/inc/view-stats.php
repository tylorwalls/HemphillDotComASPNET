...
foreach ((glob("../content/stats/*.txt")) as $fl) {
	...
	foreach ($conts as $visit) {
		$visit   = explode("|", $visit);
		if (!empty($visit[3])){ $day = trim($visit[3]);} else {$day = ""; }
		if (!empty($visit[4])){ $month   = trim($visit[4]);} else {$month = ""; }
		if (!empty($visit[0])){  $ip      = trim($visit[0]);} else {$ip = ""; }
		if (!empty($visit[1])){  $page1   = trim($visit[1]);} else {$page1 = ""; }
		if (!empty($visit[2])){  $referer = trim($visit[2]);} else {$referer = ""; }
