...
if (!(empty($get_id)) && is_numeric($get_id) && $all_blog_files[$get_id]) {	
		...
		$full_content_blog = "";
		if (preg_match_all("/".'(\\{)'.'(\\{)'.'.*?'.'(\\})'.'(\\})'."/", $content_blog, $m)){
			
			foreach ($m[0] as $this_embed){	      
				$get_embed = $this_embed;
				...
				$new  = ob_get_contents();
				$full_content_blog = str_replace($this_embed, $new, $content_blog);
				ob_end_clean();
				
				$content_blog = $full_content_blog;
			}
		}	
